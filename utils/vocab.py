from utils.params import START, END, UNKNOWN, SEC_ORDER, FST_ORDER, BAR, REMEMBER_BAR, SUF, PREF
import numpy as np


class Vocabulary:
    def __init__(self, src_file=None, labeled=True, vocab_file=None, suffix_prefix=False):
        if vocab_file:
            ...
            self._vocab_to_idx, self._idx_to_vocab, self._pref_to_idx, self._idx_to_pref, self._suf_to_idx, \
                self._idx_to_suf = self._vocab_from_train(src_file, labeled, suf_pref=suffix_prefix)
    ...
    @staticmethod
    def _vocab_from_train(src_file, labeled, suf_pref=False):
        all_words = []
        all_suf = []
        all_pref = []
...
if __name__ == "__main__":
    ...
    vc = Vocabulary(src_file=os.path.join("..", "data", "pos", "train"), suffix_prefix=True)
